...
class Space:
    ...
    def sample(self):
        ...
        if np.issubdtype(self.dtype, np.floating):
            low = np.maximum(np.ones(self.shape) *
                             np.finfo(self.dtype).min, low)
            high = np.minimum(np.ones(self.shape) *
                              np.finfo(self.dtype).max, high)
    ...
    def _infer_low(self, dtype, shape, low, high):
        if low is None:
            if np.issubdtype(dtype, np.floating):
                ...
                raise ValueError(
                    'Cannot infer low bound from shape and dtype.')
        return np.array(low)
    ...
    def _infer_high(self, dtype, shape, low, high):
        if high is None:
            if np.issubdtype(dtype, np.floating):
                ...
                raise ValueError(
                    'Cannot infer high bound from shape and dtype.')
